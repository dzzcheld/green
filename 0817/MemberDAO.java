...
public class MemberDAO {
	 ...
	 */
	/*
	 * public String selectName() { sqlMapper=getInstance(); SqlSession
	 * session=sqlMapper.openSession(); String
	 * name=(String)session.selectOne("mapper.member.selectName"); return name; }
	 * 
	 * public int selectPwd() { sqlMapper=getInstance(); SqlSession
	 * session=sqlMapper.openSession(); int
	 * pwd=(int)session.selectOne("mapper.member.selectPwd"); return pwd; }
	 */
	...
	public int insertMember2(Map<String, String> memberMap) {
		sqlMapper=getInstance();
		SqlSession session=sqlMapper.openSession();
		int result=result=result=session.insert("mapper.member.insertMember2", memberMap);
		session.commit();
		return result;
	}
	...
	public List<MemberVO> foreachSelect(List nameList){
		sqlMapper=getInstance();
		SqlSession session=sqlMapper.openSession();
		List list=session.selectList("mapper.member.foreachInsert",nameList);
		return list;
	}
